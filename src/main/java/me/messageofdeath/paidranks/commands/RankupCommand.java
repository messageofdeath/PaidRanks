			...
			if (cmd.isPlayer()) {
				if (!ladder.equalsIgnoreCase("asdfghjkl")) {
					if (this.instance.getLadderManager().hasLadder(ladder)) {
						...
						String world = ladderx.hasWorld() ? cmd.getPlayer().getWorld().getName() : null;
						if(!ladderx.hasWorld() || (ladderx.hasWorld() && ladderx.getWorld().equalsIgnoreCase(cmd.getPlayer().getWorld().getName()))) {
							...
							super.error(cmd, LanguageSettings.Commands_Rankup_InWorld.getSetting().replace("%world", ladderx.getWorld()));
						}
					} else {
